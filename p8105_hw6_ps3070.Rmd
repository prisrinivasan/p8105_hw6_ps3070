...
The 95% confidence interval for the log data is (`r round(quantile(pull(result_df, log_int_tmin), probs = c(0.025,0.975)), digits = 2)`). The 95% confidence interval for the $r^2$ is (`r round(quantile(pull(result_df, log_int_tmin), probs = c(0.025,0.975)), digits = 2)`). Since the null value of zero is not within either CI, we are 95% confident that the true values for both lie in these ranges based on this sample. The median for log(Intercept x Beta 1 Hat) is `r round(quantile(pull(result_df, log_int_tmin), probs = 0.5), digits=2)`, and the $r^2$ median is `r round(mean(pull(r2_df, r_squared)), digits=2)`.
